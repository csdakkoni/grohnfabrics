...
async function getCategories() {
  const { data } = await supabaseAdmin
    .from('categories')
    .select('id, slug, name_tr, name_en, description_tr, description_en, image_url')
    .eq('is_active', true)
    .order('sort_order')
    .limit(3);
  return data || [];
          ...
          <div className="container">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {categories.map((category) => {
                const categoryName = isEnglish ? category.name_en : category.name_tr;
                const categoryDescription = isEnglish ? category.description_en : category.description_tr;
                return (
                  <Link
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center text-center">
                      ...
                      {categoryDescription && (
                        <p className="text-sm text-[var(--foreground-muted)] font-light mb-4">{categoryDescription}</p>
                      )}
